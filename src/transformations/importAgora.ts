...
const JSON_GETTERS: Getters = {
  ...
  website: (obj: any) => normalizeUrlArray([...obj.website]),
  farcaster: (obj: any) => normalizeUrlArray([...obj.farcaster]),
  repos: (obj: any) => normalizeUrlArray([...obj.repos.map((x: any) => x.url)]),
  deployerAddress: (obj: any) =>
    trimArray([...obj.contracts.map((x: any) => x.deployerAddress)]),
};
...
const agoraToOsoProject = (agoraProj: AgoraProject): Project => {
  const result: Project = {
    version: 7,
    name: ensure(getOsoName(agoraProj.name), "Missing OSO name"),
...
async function readInput() {
  ...
  const agoraProjectsWithoutArtifacts = _.mapValues(groupByProjectId, (arr) => {
    const first = arr[0];
    return {
      agoraProjectId: GETTERS.projectId(first),
  ...
  const agoraProjectMap = _.mapValues(agoraProjectsWithoutArtifacts, (x) => ({
    ...x,
    osoProjectSlug: getOsoName(
      findOsoProject(agoraIdToArtifacts[x.agoraProjectId])?.name,
    ),
    artifacts: agoraIdToArtifacts[x.agoraProjectId],
  }));
